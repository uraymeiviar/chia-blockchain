...
async def get_plots(harvester_rpc_port: int, harvester_rpc_hostname: str) -> Optional[Dict[str, Any]]:
    plots = None
    try:
        ...
        if isinstance(e, aiohttp.ClientConnectorError):
            print(f"Connection error. Check if harvester is running at {harvester_rpc_hostname}:{harvester_rpc_port}")
        else:
...
async def get_all_plots(harvester_rpc_port: int, harvester_hosts: List[str]) -> Optional[Dict[str, Any]]:
    all_plots = None
...
async def summary(rpc_port: int, wallet_rpc_port: int, harvester_rpc_port: int, farmer_rpc_port: int) -> None:
    ...
    harvesters = await get_harvesters(farmer_rpc_port)
    plots = await get_all_plots(harvester_rpc_port, harvesters)
